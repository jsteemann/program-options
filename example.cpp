...
int main(int argc, char* argv[]) {
  ...
  std::vector<std::string> endpoints = { "tcp://127.0.0.1:80", "ssl://192.168.0.1:443" };
  ...
  ProgramOptions options(
    argv[0], 
    "Usage: " ARANGODB_PROGRAM_OPTIONS_PROGNAME " [<options>] <database-directory>",
  ...
  options.addSection(Section("", "Global options description goes here", "global options", false, false));
  options.addOption("--quiet,-q", "tell the server to be quiet", new BooleanParameter(&quiet, false));
  options.addOption("--no-server", "don't start server at all", new BooleanParameter(&noServer, false));
  options.addOption("--configuration,-c", "parse configuration file", new StringParameter(&configFile));
  options.addOption("--version", "prints version information", new ObsoleteParameter());
  

  // "server" options section
  options.addSection("server", "Server options description goes here");
  options.addOption("--server.endpoints,-e", "server endpoints", new VectorParameter<StringParameter>(&endpoints));
  options.addOption("--server.ports", "the server ports", new VectorParameter<PortParameter>(&ports));
  options.addOption("--server.int32-value", "an int32 value", new Int32Parameter(&int32));
  options.addOption("--server.uint32-value", "a uint32 value", new UInt32Parameter(&uint32));
  options.addOption("--server.bounded-value", "a bounded uint32 value", new BoundedParameter<UInt32Parameter>(&bounded, 42, 8193));


  // "database" options section
  options.addSection("database", "Database options description goes here");
  options.addOption("--database.journal-size", "maximal journal size", new UInt32Parameter(&journalSize));
  options.addOption("--database.wait-for-sync", "wait for sync description", new BooleanParameter(&waitForSync));


  // hidden section
  options.addHiddenSection("debugging", "Debugging options description goes here");
  options.addOption("--debugging.crash-me", "whatever (option can still be used but it is not shown)", new BooleanParameter(&crashMe));
  options.addObsoleteOption("--debugging.not-used-anymore", "whatever (obsolete)");
  ...
  if (!configFile.empty()) {  
    ...
    std::cout << "Parsing config file '" << configFile << "'..." << std::endl << std::endl;
  ...
  std::cout << "Positional arguments (" << positionals.size() << "):" << std::endl;
  for (auto const& it : positionals) {
    std::cout << "- positional: '" << it << "'" << std::endl;
  } 
  std::cout << std::endl; 

  // print all (touched) options
  std::cout << "Touched options:" << std::endl;
  options.walk([] (Section const& section, Option const& option) {
    std::cout << "- section: '" << section.name << 
                 "', option: '" << option.name << 
                 "', full name: '" << option.displayName() << 
                 "', type: '" << option.parameter->typeDescription() << 
                 "', value: '" << option.parameter->valueString() << "'" << std::endl;
  }, true);
  std::cout << std::endl;
}
